...
class RoarCompetitionRule:
    ...
    async def tick(
        ...
        speed = np.linalg.norm(self.vehicle.get_linear_3d_velocity())

        #print(f"reach waypoints {self.furthest_waypoints_index} at {self.waypoints[self.furthest_waypoints_index].location}")
        print(f"Reached waypoint {self.furthest_waypoints_index} at "
        f"{self.waypoints[self.furthest_waypoints_index].location} | "
        f"Speed: {speed:.2f} m/s")
        #current_time = time.time()
        #print("Solution took " + str(current_time-start_time))
...
async def main():
    ...
    if evaluation_result is not None:
        ...
        print("Solution took {} seconds".format(evaluation_result["elapsed_time"]))
