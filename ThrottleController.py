...
class ThrottleController:
    ...
    def speed_data_to_throttle_and_brake(self, speed_data: SpeedData):
        ...
        if percent_of_max > 1:
            ...
            if speed_data.current_speed > 200:  # Brake earlier at higher speeds
                print(
                    f" the current speed is {speed_data.current_speed}, the recommended speed {speed_data.recommended_speed_now} "
                )
    ...
    def get_target_speed(self, radius: float, current_section: int):
        ...
        mu = get_friction(current_section)
        ...
        if self.display_debug:
            print(
                f"[SpeedCalc] Sec {current_section} | Radius: {round(radius,1)} | mu: {mu} | TargetSpeed: {round(target_speed,1)}"
            )
